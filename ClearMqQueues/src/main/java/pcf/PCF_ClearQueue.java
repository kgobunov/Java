 ...
 * The sample demonstrates how PCF commands can be used to clear messages from a
 * queue.
 * <p>
 ...
 * <br>
 * A typical output when running this sample when a valid queue is present would
 * be:- <br>
 * <br>
...
public class PCF_ClearQueue {
	...
	/**
	 * ClearQueue assumes that a queue has been created (by previously calling
	 * the CreateQueue sample). The queue is then filled with a random number of
	 * random length messages. The PCF command 'MQCMD_INQUIRE_Q' is then called
	 * to find how many messages are in the queue. Then the PCF command
	 * 'MQCMD_CLEAR_Q' is used to clear all messages from the Queue. Finally a
	 * second 'MQCMD_INQUIRE_Q' command is used to show that the queue has been
	 * cleared. <br>
	 * For more information on the Create Queue command, please read the
	 ...
	 * @throws PCFException
	 */
	public static void ClearQueue(PCF_CommonMethods pcfCM) throws MQException,
			PCFException, MQDataException, IOException {
		...
		// There will
		// only be one set of queue status information, but this shows how it
		// could be extended.
		...
		System.out
				.println("+-----+------------------------------------------------+-----+");
		System.out
				.println("|Index|             Queue Name (before)                |Depth|");
		System.out
				.println("+-----+------------------------------------------------+-----+");
		...
		for (int index = 0; index < pcfResponse.length; index++) {
			...
			System.out
					.println("|"
							...
							+ "|"
							+ (response
									.getParameterValue(MQConstants.MQCA_Q_NAME) + pcfCM.padding)
									.substring(0, 48)
							+ "|"
							+ (response
									.getParameterValue(MQConstants.MQIA_CURRENT_Q_DEPTH) + pcfCM.padding)
									.substring(0, 5) + "|");
		}
		...
		// The sample has so far connected to the Queue and filled it with a
		// random number of
		// random length messages. The sample has demonstrated that these
		// messages are on the
		// Queue. Now clear the queue and then demonstrate that the queue is
		// empty.
		// Clear the messages. Note that rather than create another PCFMessage,
		// the existing
		...
		System.out
				.println("+-----+------------------------------------------------+-----+");
		System.out
				.println("|Index|             Queue Name (after)                 |Depth|");
		System.out
				.println("+-----+------------------------------------------------+-----+");
