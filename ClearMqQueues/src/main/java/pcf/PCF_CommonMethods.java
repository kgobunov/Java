 ...
 * The PCF samples can be run independently or in conjunction with each other.
 * An example of how they can be linked together is shown below:-<br>
 * <br>
 ...
 * </ul>
 * <br>
 * <b>Note that this sample cannot be run independently (there is no main
 * method), but this class must be included by all other PCF samples in this
 * suite.</b>
 * <p>
 ...
 * firewall has been prepared to allow this connection.</b>
 */
...
public class PCF_CommonMethods {
	...
	/**
	 * Constant used to identify the PCF queue name that will be generated/used
	 * by some of the PCF samples.
	 */
//	public static final String pcfQueue = "PCFQUEUE";
	
	
	public static final String pcfQueue = "APLANA.TEST";

	/**
	 * Constant used to identify the PCF channel name that will be
	 * generated/used by some of the PCF samples.
	 */
	public static final String pcfChannel = "PCFCHANL";

	/**
	 * If used, will contain the hostname of the machine that is running the
	 * queue manager. For a queue manager running on the local machine this
	 * would be localhost or 127.0.0.1.
	 */
	public String host = "10.67.4.25";

	/**
	 * If used, will contain the port of the queue listener. The default is
	 * usually 1414.
	 */
	 ...
	 */
	public String queueManager = "M00.MOSCOW";
	...
	/**
	 * Boolean used to indicate whether the connection is using client or server
	 * bindings.
	 */
	 ...
	 * <b>Note that this sample cannot be run independently, but this class must
	 * be included by all other PCF samples in this suite.</b><br>
	 * The PCF samples can be run independently or in conjunction. An example of
	 * how they can be linked together is shown below:-<br>
	...
	/**
	 * Input parameter parser. This method looks at the input parameter list.
	 * The PCF sample can be called with either just a queue manager name or
	 * with queue manager, IP address of the machine running the queue manager
	 * and the port on which it is listening. If just the queue manager is
	 * defined, then the sample will use local bindings. If the queue manager
	 * name, host and port are defined, then the sample will use client
	 * bindings.
	 * 
	 * @param args
	...
	public boolean ParseParameters(String[] args) {
		...
		if (((args.length == 1) && (flags == 1))
				|| ((args.length > 2) && (flags == 7))) {
			return true;
		}

		System.out
				.println("The PCF samples must be used with the following parameters:-");
	 ...
	 */
	public void CreateAgent() throws MQDataException {
		try {
			...
			if (mqde.reasonCode == CMQC.MQRC_Q_MGR_NAME_ERROR) {
				...
				if (this.client) {
					...
					System.out.print("default queue manager at \"" + host
							+ "\", port \"" + port + "\"");
				} else {
				...
				System.out.println(" or could not find the default channel \""
	...
	/**
	 * Destroy the agent. This method is used by all PCF samples to destroy the
	 * PCFAgent.
	 * 
	 * @throws MQDataException
	 */
	...
	public void DisplayException(Exception exception) {
		if (exception.getClass().equals(PCFException.class)) {
			...
			if (pcfe.reasonCode == MQConstants.MQRCCF_CHANNEL_NOT_FOUND) {
				if (client) {
					System.out.print("Either the queue manager \""
							+ queueManager + "\"");
				} else {
				...
				System.err.println(pcfe + ": "
						+ MQConstants.lookupReasonCode(pcfe.reasonCode));
			}
			...
			System.err.println(de + ": "
					+ MQConstants.lookupReasonCode(de.reasonCode));
			...
			if (mqe.reasonCode == CMQC.MQRC_UNKNOWN_OBJECT_NAME) {
				System.out.println("Cound not find the queue \"" + pcfQueue
						+ "\" on the queue manager \"" + queueManager + "\".");
			} else {
				System.err.println(mqe + ": "
						+ MQConstants.lookupReasonCode(mqe.reasonCode));
			}
		}
